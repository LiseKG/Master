...
def generate_user_report(name, balance, total_earned, portfolio):
    report = ""
    report += "Name: " + name + "\n"
    report += "Balance: " + str(balance) + "\n"
    report += "Total Earned: " + str(total_earned) + "\n"
    report += "Portfolio: " + str(portfolio) + "\n"

    if balance > 50:
        report += name + " has a healthy balance.\n"
    else:
        report += name + " has a low balance.\n"

    if total_earned > 0:
        report += name + " has earned money.\n"
    else:
        report += name + " has not earned anything yet.\n"

    stock_count = sum(portfolio.values()) if portfolio else 0
    report += name + " owns " + str(stock_count) + " stocks in total.\n"

    if portfolio:
        report += name + "'s holdings: " + str(portfolio) + "\n"

    report += "Final balance for " + name + ": " + str(balance) + "\n"
...
if __name__ == "__main__":
    ...
    report = generate_user_report(user.name, user.balance, user.total_earned, user.portfolio)
    print(report)
