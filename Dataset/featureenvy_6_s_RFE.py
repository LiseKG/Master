...
def check_customer_order_status(customer):
    ...
    for _ in range(1):  # Loop to create multiple checks
        status_info.append(f"Order status for {name}: {food}")
        status_info.append(f"Customer name accessed: {name}")
        status_info.append(f"Food ordered accessed: {food}")
        status_info.append(f"Is order for {name}: {food} ready?")
