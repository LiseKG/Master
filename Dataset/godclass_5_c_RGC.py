...
class EmailComposer:
    def __init__(self, event_log, mailbox):
    ...
    def send_email(self, sender: User, receiver: User, content: str):
        ...
        self.event_log.append(f"Email sent from {sender.email_address} to {receiver.email_address}")
    ...
    def send_email_with_image(self, sender: User, receiver: User, content: str, image_name: str):
        ...
        mail.has_image = True
        self.event_log.append(f"Image '{image_name}' attached to email for {receiver.email_address}")
    ...
    def send_bulk_emails(self, sender: User, receivers: list, content: str):
        ...
        self.event_log.append(f"Bulk email sent by {sender.email_address} to {len(receivers)} recipients.")
    ...
    def forward_email(self, original: Email, sender: User, new_receiver: User):
        ...
        self.event_log.append(f"Email forwarded to {new_receiver.email_address}")
        return mail
...
class MailboxManager:
    ...
    def mark_as_read(self, mail: Email):
        mail.read = True
        self.event_log.append(f"Email from {mail.sender_address} marked as read.")
    ...
    def delete_email(self, user: User, index: int):
        ...
        self.event_log.append(f"Email from {removed.sender_address} deleted from {user.email_address}'s mailbox.")
    ...
    def clear_mailbox(self, user: User):
        ...
        user.email = []
        self.event_log.append(f"Mailbox cleared for {user.email_address} ({count} emails removed).")
...
class EmailSystem:
    def __init__(self):
        ...
        self.users = {}
        self.event_log = []
        self.composer = EmailComposer(self.event_log, self.mailbox)
        self.manager = MailboxManager(self.event_log, self.mailbox)
...
if __name__ == "__main__":
    ...
    system.composer.send_email(alice, jon, "hi there!")
    system.composer.send_email(jon, alice, "Hello back!")

    # Method 7 - send_email_with_image (calls send_email, log_event)
    system.composer.send_email_with_image(alice, jon, "Check this out!", "photo.jpg")

    # Method 8 - get_mailbox (calls view_mailbox on User)
    system.manager.get_mailbox(jon)

    # Method 13 - track_email
    status = system.manager.track_email(alice, jon, "hi")
    print(f"Track status: {status}")

    # Method 14 - check_email_contains (calls track_email)
    system.manager.check_email_contains(alice, jon, "hi")

    # Method 10 - show_new_emails (calls get_unread_emails)
    system.manager.show_new_emails(jon)

    # Method 12 - read_email (calls mark_as_read which calls log_event)
    system.manager.read_email(jon, 0)

    # Method 15 - forward_email (calls send_email, log_event)
    original = jon.email[0]
    system.composer.forward_email(original, jon, bob)

    # Method 16 - delete_email (calls log_event)
    system.manager.delete_email(jon, 0)

    # Method 17 - send_bulk_emails (calls send_email, log_event)
    system.composer.send_bulk_emails(alice, [jon, bob], "Hello everyone!")

    # Method 18 - show_mailbox_summary (calls get_unread_emails)
    system.manager.show_mailbox_summary(jon)
    system.manager.show_mailbox_summary(bob)

    # Method 19 - search_inbox
    system.manager.search_inbox(jon, "Hello")

    # Method 21 - read_all_emails (calls get_unread_emails, mark_as_read)
    system.manager.read_all_emails(jon)

    # Method 22 - clear_mailbox (calls log_event)
    system.manager.clear_mailbox(bob)

    # Method 20 - show_event_log
    system.show_event_log()

    # Method 9 - get_unread_emails (called by others but also directly)
    unread = system.manager.get_unread_emails(alice)
    print(f"Alice unread count: {len(unread)}")

    # Method 11 - mark_as_read (called by others but also directly)
    if alice.email:
        system.manager.mark_as_read(alice.email[0])

    # Methods 3 and 4 called via send_email above; call directly to ensure coverage
    mail = system.composer.create_email("x@mail.com", "y@mail.com", "Test", "Test content")
    print(f"Created email subject: {system.composer.format_subject('  test subject  ')}")
    print(f"Email has image: {mail.has_image}")

    # Method 5 - deliver_email (called via send_email above; call directly too)
    test_mail = system.composer.create_email("a@mail.com", "b@mail.com", "Direct", "Direct delivery")
    system.composer.deliver_email(jon, test_mail)
