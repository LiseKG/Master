class MovieStreamingPlatform:
    ...
    def rent_movie(self, movie_name):
        movie = next((m for m in self.available_movies if m['title'] == movie_name and m['status'] == 'available'), None)
        if movie:
            movie['status'] = 'rented'
    ...
    def return_movie(self, movie_name):
        movie = next((m for m in self.movies_rented if m['title'] == movie_name), None)
        if movie:
    ...
    def get_movie_info(self, movie_name):
        movie = next((m for m in self.available_movies if m['title'] == movie_name), None)
        if movie:
    ...
    def is_movie_available(self, movie_name):
        return any(movie for movie in self.available_movies if movie['title'] == movie_name and movie['status'] == 'available')
    ...
    def clear_rented_movies(self):
        self.movies_rented.clear()

    def movie_exists(self, title):
        return any(movie['title'] == title for movie in self.available_movies)

    def get_movie_status(self, title):
        movie = next((m for m in self.available_movies if m['title'] == title), None)
        if movie:
