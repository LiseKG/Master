...
def generate_user_email_report(email_address, email):
    report = ""
    report += "Email Address: " + email_address + "\n"
    report += "Total emails: " + str(len(email)) + "\n"

    if len(email) == 0:
        report += email_address + " has no emails.\n"
    else:
        report += email_address + " has " + str(len(email)) + " email(s).\n"

    report += "Listing emails for " + email_address + ":\n"
    for mail in email:
        report += "  - From: " + mail["from"] + " | Content: " + mail["content"] + "\n"

    if email:
        report += "First email received by " + email_address + ": " + email[0]["content"] + "\n"
        report += "Last email received by " + email_address + ": " + email[-1]["content"] + "\n"

    report += "Summary for " + email_address + ": inbox size = " + str(len(email)) + "\n"
...
if __name__ == "__main__":
    ...
    report = generate_user_email_report(jon.email_address, jon.email)
    print(report)
