...
def analyze_totals(user, earnings):
...
def calculate_earnings(user):
    return sum(user.portfolio[stock] * 5 for stock in user.portfolio)


def examine_stocks(user):
    for stock, quantity in user.portfolio.items():
        print(f"Owned: {quantity} of {stock}")
        print(f"Checking stock {stock}")
    check_specific_stock("EN")
    print("Verifying stock ownership... Done.")
...
def calculate_average_price(user):
    if not user.portfolio:
        ...
        average_price = sum(price_recorded) / len(price_recorded)
...
def final_checks(user):
    ...
    total_stocks = sum(user.portfolio.values())
    print("Final balance check." if total_stocks > 0 else "No stocks to check.")
