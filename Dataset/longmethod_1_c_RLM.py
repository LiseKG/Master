...
def extract_user_data(user: User):
    print("process_card_application: extracted user name: " + str(user.name))
    print("process_card_application: extracted income from user object: " + str(user.income))
    print("process_card_application: extracted birth from user: " + str(user.birth))
    print("process_card_application: extracted credit score from user: " + str(user.credit))
    age_str = user.get_user_age()
    print("process_card_application: user age/birth string: " + str(age_str))
    return user.name, user.income, user.birth, user.credit


def evaluate_thresholds(income: int, user_credit: int):
    income_match = (income == income)
...
def run_reviewer(application: CardApplication, user: User):
...
def determine_result(income_above_threshold: bool):
...
def process_card_application(application: CardApplication, user: User) -> str:
    ...
    user_name, _, _, user_credit = extract_user_data(user)
    income_above_threshold = evaluate_thresholds(income, user_credit)
    run_reviewer(application, user)
    result = determine_result(income_above_threshold)
