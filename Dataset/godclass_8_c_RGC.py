...
class User:
    ...
    def give_likes(self, platform, picture_name):
        platform.picture_manager.add_like(self, picture_name)
...
class UserManager:
    ...
    def register_and_like(self, username, picture_name):
        ...
        self.picture_manager.add_like(user, picture_name)
        return user
...
class PlatformReporter:
    def __init__(self, user_manager: UserManager, picture_manager: PictureManager):
        self.user_manager = user_manager
        self.picture_manager = picture_manager

    def show_picture_details(self, picture_name):
        picture = self.picture_manager.find_picture(picture_name)
        if picture is None:
            print(f"Picture '{picture_name}' not found.")
            return
    ...
    def show_all_pictures(self, user: User):
        user_pics = [p for p in self.picture_manager.pictures.values() if p.owner_username == user.username]
    ...
    def get_most_liked(self):
        if not self.picture_manager.pictures:
            print("No pictures available.")
            return None
        top = max(self.picture_manager.pictures.values(), key=lambda p: len(p.likes))
    ...
    def show_platform_summary(self):
        total_users = len(self.user_manager.platform)
        total_pics = len(self.picture_manager.pictures)
        total_likes = sum(len(p.likes) for p in self.picture_manager.pictures.values())
...
class SocialMediaPlatform:
    def __init__(self):
        ...
        self.user_manager = UserManager(self.picture_manager)
        self.reporter = PlatformReporter(self.user_manager, self.picture_manager)
...
if __name__ == "__main__":
    ...
    platform.user_manager.register_user(alice)
    platform.user_manager.register_user(jon)
    platform.user_manager.register_user(chris)

    # Method 10 - post_two_pictures (calls add_picture x2 -> create_picture -> log_action)
    platform.picture_manager.post_two_pictures(alice, "sunset.jpg", "beach.jpg")

    # Method 6 - add_like (calls find_picture, log_action)
    platform.picture_manager.add_like(jon, "sunset.jpg")
    platform.picture_manager.add_like(chris, "sunset.jpg")

    # User.give_likes calls platform.picture_manager.add_like
    jon.give_likes(platform, "beach.jpg")
    chris.give_likes(platform, "beach.jpg")

    # Method 8 - show_total_likes (calls find_picture)
    platform.picture_manager.show_total_likes(alice, "sunset.jpg")
    platform.picture_manager.show_total_likes(alice, "beach.jpg")

    # Method 7 - remove_like (calls find_picture, log_action)
    platform.picture_manager.remove_like(jon, "sunset.jpg")
    platform.picture_manager.show_total_likes(alice, "sunset.jpg")

    # Method 9 - show_picture_details (calls find_picture)
    platform.reporter.show_picture_details("sunset.jpg")

    # Method 12 - bulk_like (calls register_and_like -> register_user + add_like, log_action)
    platform.user_manager.bulk_like(["User1", "User2", "User3", "User4", "User5"], "beach.jpg")

    # Method 13 - show_all_pictures (calls find_picture indirectly via list comp)
    platform.reporter.show_all_pictures(alice)

    # Method 15 - transfer_likes (calls find_picture, log_action, show_total_likes)
    platform.picture_manager.transfer_likes("beach.jpg", "sunset.jpg")
    ...
    platform.user_manager.register_and_like("ExtraUser", "sunset.jpg")

    # Method 5 - find_picture (standalone, called directly)
    pic = platform.picture_manager.find_picture("sunset.jpg")
