...
class Product:
    ...
    def generate_report(self):
        ...
        report += "Price: $" + str(self.price) + "\n"
        report += "Info: " + self.get_info() + "\n"
        if self.price > 100:
            report += self.name + " is expensive\n"
        else:
            report += self.name + " is affordable\n"
        discounted = self.price * 0.9
        report += "Discounted price for " + self.name + ": $" + str(discounted) + "\n"
        tax = self.price * 0.2
        report += "Tax for " + self.name + ": $" + str(tax) + "\n"
        total = self.price + tax
        report += "Total with tax: $" + str(total) + "\n"
        report += "Summary: " + self.get_info() + " (tax included: $" + str(total) + ")\n"
        return report
