...
class CardApplicationReviewer():
    ...
    def review(self, application: CardApplication) -> str:
        ...
        """
        return CardApplicationResult.APPROVED if application.income > 1000 else CardApplicationResult.DECLINED


def evaluate_multiple_applications(applications):
    results = []
    reviewer = CardApplicationReviewer()  # Moved reviewer instantiation outside loop to improve efficiency

    for app in applications:
        review_result = reviewer.review(app)

        approval_status = review_result == CardApplicationResult.APPROVED
        income_check = app.income if app.income > 800 else 0
        ...
        result = {
            ...
            "application_status": f"{app.income}: {review_result}",
            "application_income_check": income_check
        }
